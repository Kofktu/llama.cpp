...
struct gguf_reader_buffer {
    ...
    bool read(std::string & dst) {
        ...
        // 1. 문자열 길이 읽기 (uint64_t)
        // 위에서 정의한 read(void*, size)를 재사용합니다.
...
bool gguf_read_buffer_emplace_helper(struct gguf_reader_buffer & gr, std::vector<struct gguf_kv> & kv, const std::string & key, const bool is_array, const size_t n) {
    // [Case 1] 배열(Array)인 경우 -> 최적화된 Scan-then-Copy 적용
    // 이유: Tokenizer 같은 대용량 배열 로딩 시 메모리 폭발 방지 및 속도 향상
    if (is_array) {
        ...
        if constexpr (std::is_same_v<T, std::string>) {
            ...
            values.reserve(n); // 메모리 재할당 방지 최적화
            
            // String Array : 길이 정보를 읽으며 스캔
            for (size_t i = 0; i < n; ++ i) {
                uint64_t len;
